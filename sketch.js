...
function draw() {
	...
	if (stage2_trigger){
		push();
		filter(INVERT);
		lightning.animate(0.9, target.x - (target.y*0.9)/2 + 40, 0, target.y*0.9, target.y*0.9);
...
function setup_logo() {
	target = createVector(windowWidth/2 - 52, windowHeight/2 - 72);
